...
def getJsonUrl(url):
    Data = urllib.request.urlopen(urllib.request.Request(url=url, headers=headers)).read().decode('utf-8')
...
def start():
    ...
    if sys.getdefaultencoding() == 'ascii':
        sys.stdout = open("favorites.txt", "w", encoding='gb2312')
    else:
        sys.stdout = open("favorites.txt", "w", encoding='utf-8')
